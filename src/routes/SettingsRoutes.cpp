...
void WebServerManager::setupSettingsRoutes() {
    ...
    server.on(
        "/api/settings/wifi", HTTP_POST, []([[maybe_unused]] AsyncWebServerRequest* request) {}, nullptr,
    ...
    server.on(
        "/api/settings/device-name", HTTP_POST, []([[maybe_unused]] AsyncWebServerRequest* request) {}, nullptr,
        [this](AsyncWebServerRequest* request, uint8_t* data, size_t len, size_t index, [[maybe_unused]] size_t total) {
            if (index == 0) {
                if (!verifyCsrfHeader(request)) {
                    return;
    ...
    server.on(
        "/api/settings/elevation", HTTP_POST, []([[maybe_unused]] AsyncWebServerRequest* request) {}, nullptr,
        [this](AsyncWebServerRequest* request, uint8_t* data, size_t len, size_t index, [[maybe_unused]] size_t total) {
            if (index == 0) {
                if (!verifyCsrfHeader(request)) {
                    return;
                ...
                if (!doc["elevation"].is<float>()) {
                    request->send(400, CONTENT_TYPE_JSON, R"({"success":false,"error":"Elevation required"})");
                    return;
                }
    ...
    server.on(
        "/api/settings/timezone", HTTP_POST, []([[maybe_unused]] AsyncWebServerRequest* request) {}, nullptr,
        [this](AsyncWebServerRequest* request, uint8_t* data, size_t len, size_t index, [[maybe_unused]] size_t total) {
            if (index == 0) {
                if (!verifyCsrfHeader(request)) {
                    return;
    ...
    server.on("/api/settings/factory-reset", HTTP_POST, [this](AsyncWebServerRequest* request) {
        ...
        if (confirm == nullptr || confirm->value() != "factory-reset") {
            ...
            request->send(400, CONTENT_TYPE_JSON, R"({"success":false,"error":"Missing confirmation"})");
            return;
        }
    ...
    server.on(
        "/api/settings/energy", HTTP_POST, []([[maybe_unused]] AsyncWebServerRequest* request) {}, nullptr,
        [this](AsyncWebServerRequest* request, uint8_t* data, size_t len, size_t index, [[maybe_unused]] size_t total) {
            if (index == 0) {
                if (!verifyCsrfHeader(request)) {
                    return;
                ...
                const bool wifiChanged =
                    energyConfig.wifi_power != prevWifiPower || energyConfig.wifi_sleep_mode != prevSleepMode;

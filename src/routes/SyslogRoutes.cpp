...
void WebServerManager::setupSyslogRoutes() {
    ...
    server.on(
        "/api/syslog", HTTP_POST, []([[maybe_unused]] AsyncWebServerRequest* request) {}, nullptr,
        [this](AsyncWebServerRequest* request, uint8_t* data, size_t len, size_t index, [[maybe_unused]] size_t total) {
            if (index == 0) {
                ...
                if (doc["host"].is<const char*>())
                    strlcpy(syslogConfig.host, doc["host"] | "", sizeof(syslogConfig.host));
                if (doc["port"].is<int>()) syslogConfig.port = doc["port"];

                config.saveSyslogConfig(syslogConfig);
                SyslogOutput::setConfig(syslogConfig);

                ESP_LOGI(TAG, "Syslog config updated: enabled=%d host=%s port=%u", syslogConfig.enabled,
                         syslogConfig.host, syslogConfig.port);

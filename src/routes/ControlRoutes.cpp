...
namespace {
    ...
    const char* autotuneStateName(Control::AutotuneState state) {
        switch (state) {
            case Control::AutotuneState::Idle:
                return "idle";
            case Control::AutotuneState::Settling:
                return "settling";
            case Control::AutotuneState::Oscillating:
                return "oscillating";
            case Control::AutotuneState::Done:
                return "done";
            case Control::AutotuneState::Aborted:
                return "aborted";
        }
        return "unknown";
    }
    ...
    const char* autotuneAbortName(Control::AutotuneAbort reason) {
        switch (reason) {
            case Control::AutotuneAbort::None:
                return "none";
            case Control::AutotuneAbort::UserRequested:
                return "user_requested";
            case Control::AutotuneAbort::CeilingBreached:
                return "ceiling_breached";
            case Control::AutotuneAbort::FloorBreached:
                return "floor_breached";
            case Control::AutotuneAbort::SensorLost:
                return "sensor_lost";
            case Control::AutotuneAbort::RunTimeout:
                return "run_timeout";
            case Control::AutotuneAbort::SettlingTimeout:
                return "settling_timeout";
            case Control::AutotuneAbort::AmplitudeTooSmall:
                return "amplitude_too_small";
            case Control::AutotuneAbort::DerivedGainsInvalid:
                return "derived_gains_invalid";
        }
        return "unknown";
    }
}
...
void WebServerManager::setupControlRoutes() {
    ...
    server.on(
        AsyncURIMatcher::exact("/api/control/tuning"), HTTP_POST,
        []([[maybe_unused]] AsyncWebServerRequest* request) {}, nullptr,
        [this](AsyncWebServerRequest* request, uint8_t* data, size_t len, size_t index, [[maybe_unused]] size_t total) {
            ...
            temperatureController.requestGains(Control::PidGains{kp, ki, kd}, static_cast<uint16_t>(intervalS));
            request->send(200, CONTENT_TYPE_JSON, JSON_RESPONSE_SUCCESS);
        });

    // POST /api/temperature/target - Set target temperature
    server.on(
        "/api/temperature/target", HTTP_POST, []([[maybe_unused]] AsyncWebServerRequest* request) {}, nullptr,
        [this](AsyncWebServerRequest* request, uint8_t* data, size_t len, size_t index, [[maybe_unused]] size_t total) {
            if (index == 0) {
                if (!verifyCsrfHeader(request)) {
                    return;
                ...
                if (!doc[JSON_KEY_VALUE].is<float>()) {
                    request->send(400, CONTENT_TYPE_JSON, R"({"success":false,"error":"Value required"})");
                    return;
                }
                ...
                // returns 30.0f.
                if (!std::isfinite(targetTemp) || targetTemp < Config::TARGET_TEMPERATURE_MIN_C ||
    ...
    server.on(
        AsyncURIMatcher::exact("/api/actuator"), HTTP_POST, []([[maybe_unused]] AsyncWebServerRequest* request) {},
        nullptr,
        [this](AsyncWebServerRequest* request, uint8_t* data, size_t len, size_t index, [[maybe_unused]] size_t total) {
            if (index != 0) {
                return;
            ...
            if (host[0] != '\0' && (ch < 0 || ch > static_cast<int>(Config::MAX_ACTUATOR_CHANNEL))) {
                request->send(400, CONTENT_TYPE_JSON, R"({"success":false,"error":"channel must be 0-3"})");
                return;
            }
    ...
    server.on(
        "/api/actuator/timing", HTTP_POST, []([[maybe_unused]] AsyncWebServerRequest* request) {}, nullptr,
        [this](AsyncWebServerRequest* request, uint8_t* data, size_t len, size_t index, [[maybe_unused]] size_t total) {
            ...
            if (!std::isfinite(safeMax) || safeMax < Config::MIN_SAFETY_MAX_C || safeMax > Config::MAX_SAFETY_MAX_C ||
                !std::isfinite(safeHyst) || safeHyst <= 0.0f || safeHyst > 10.0f) {
                request->send(400, CONTENT_TYPE_JSON,
                              R"({"success":false,"error":"safety limit or hysteresis out of range"})");
                return;
            }
            Support::markStage(Support::StageValidated);
            config.updateActuatorTiming(static_cast<uint16_t>(cycleS), static_cast<uint16_t>(travelS), safeMax,
                                        safeHyst);
            request->send(200, CONTENT_TYPE_JSON, JSON_RESPONSE_SUCCESS);
    ...
    server.on("/api/autotune/status", HTTP_GET, [this](AsyncWebServerRequest* request) {
        ...
        if (!canConverge) {
            doc["converge_note"] =
                act.isAssigned()
                    ? "The assigned actuator channel is refused, so nothing can drive the room and a run will end in a "
                      "timeout."
    ...
    server.on("/api/autotune/accept", HTTP_POST, [this](AsyncWebServerRequest* request) {
        ...
        if (!temperatureController.acceptAutotuneResult()) {
            request->send(409, CONTENT_TYPE_JSON, R"({"success":false,"error":"No converged result to accept"})");
            return;
        }
        request->send(200, CONTENT_TYPE_JSON, R"({"success":true,"accepted":true,"persisted":true})");
    });

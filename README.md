...
The complete API specification is available in [OpenAPI format](docs/api/klimacontrol-api.yaml) with interactive HTML documentation generated by GitHub Actions.
...
For complete API details including request/response schemas, see the [OpenAPI specification](docs/api/klimacontrol-api.yaml).
...
```bash
npm run generate:openspec-docs
```

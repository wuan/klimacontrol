...
- [ ] 1.1 Add `kp`, `ki`, `kd` and `control_interval_s` to
...
- [ ] 1.2 Set the new defaults per design D5: `kp = 0.5`, `ki = 0.0001`,
...
- [ ] 1.3 Add NVS keys `"pid_kp"`, `"pid_ki"`, `"pid_kd"`, `"ctrl_intv"` as
      private `ConfigManager` constants (the `Config.h:258+` convention, not
      `PrefsKeys.h`), each with a `static_assert(nvsKeyFits(...))`
- [ ] 1.4 Extend `validateDeviceConfig()`: per-field range check with fallback
      to the default on non-finite or out-of-range, and reject `kp == 0`
- [ ] 1.5 Load and save the four fields in `loadDeviceConfig()` /
      `saveDeviceConfig()`
- [ ] 1.6 Add `ConfigManager::updateTuning(kp, ki, kd, intervalS)` as a partial
      update in the shape of `updateActuatorTiming()`
- [ ] 1.7 Tests (`pio test -e native`): per-field range validation; NaN and
      infinity fall back; `kp == 0` falls back; `ki == 0` and `kd == 0` are
      preserved; round-trip through `updateTuning()`

## 2. Control loop decimation

- [ ] 2.1 Add a `lastPidComputeMs` member to `SensorController` and guard the
...
- [ ] 2.2 **Verify by reading the code** that the guard encloses only
      `pid.update()` — not the over-temperature shutoff at the top of
      `updateControl()`, and not the `autotuner.update()` branch (design D1, D2)
- [ ] 2.3 On a non-computing tick, leave `lastControlOutput` at the last
...
- [ ] 2.4 Reseat `lastPidComputeMs` wherever `pid.suspend()` is called, so a
      resumed controller does not immediately compute against a stale interval
      baseline
- [ ] 2.5 Tests: PID computes once per N sensor ticks; the safety shutoff
...
- [ ] 3.1 Add an atomic pending-gains request to `SensorController`, following
      the `autotuneStartRequested` / `autotuneCancelRequested` pattern
- [ ] 3.2 Consume the request at the top of `updateControl()` and call
      `pid.setGains()` from there — the control task — so the web task never
      touches PID state (design D4)
- [ ] 3.3 Change `acceptAutotuneResult()` to persist via `updateTuning()` and
      raise the pending-gains request instead of calling `pid.setGains()`
      directly. Persist before requesting (design D7)
- [ ] 3.4 Add a `requestGains()` entry point for the tuning endpoint that raises
      the same request
- [ ] 3.5 Delete the now-false "In memory only: DeviceConfig has no gain fields"
      comment in `acceptAutotuneResult()`
- [ ] 3.6 Re-read `PidController.h:58-62` — the comment asserting the web task
      never touches PID state — and confirm it is true again. Update it if the
      wording needs to name the request path
- [ ] 3.7 Push config gains into `PidController` at construction or first tick,
      so a stored tuning is in force after a restart rather than the defaults
- [ ] 3.8 Tests: a requested gain change is not visible until a tick has run;
...
- [ ] 4.1 Add `POST /api/control/tuning` in `src/routes/ControlRoutes.cpp` with
      `verifyCsrfHeader()`, all-or-nothing validation, and an error naming the
      offending field
- [ ] 4.2 Register it with `AsyncURIMatcher::exact` — the existing
...
- [ ] 4.3 Extend `GET /api/control` with `control_interval_s`, and confirm the
...
- [ ] 4.4 Verify the endpoint rejects rather than clamps, and returns the
      correct status code for a validation failure per the project's convention
- [ ] 4.5 Confirm `POST /api/autotune/accept` still answers immediately and that
...
- [ ] 5.1 Add a Tuning section to `data/settings.html`: four fields with units
      and ranges, its own save action, the derived `Ti = Kp / Ki` in seconds,
      and the note that these values drive a physical valve
- [ ] 5.2 Handle `Ki = 0` in the `Ti` display without dividing by zero
- [ ] 5.3 Surface a rejected save with the offending field named, leaving the
      entered values in the form
- [ ] 5.4 Remove the "applied in memory only / lost on restart" wording from the
      autotune acceptance UI
- [ ] 5.5 Make the "runs cannot converge" warning conditional on the actuator
      ...
      assigned
- [ ] 5.6 Regenerate `src/generated/settings_gz.h` (and `control_gz.h` if
...
- [ ] 6.2 `pio run -e adafruit_qtpy_esp32s2` — builds
...
- [ ] 6.5 Update the README's temperature-control section with the new defaults
...
- [ ] 6.6 Reconsider the design's open questions in light of what the bench unit
      ...
      the UI at all
- [ ] 6.7 `/opsx:verify`, then `/opsx:archive`

...
- `DisplayConfig` — `enabled` (default `false`), `rotation` (0..3, default 0), and `interval` (minimum seconds between e-paper refreshes, 10..3600, default 60).
...
NVS keys SHALL remain within the ≤12-character limit documented in `PrefsKeys.h`. The display keys SHALL therefore be `disp_enabled`, `disp_rot` and `disp_intv`; the `interval` field SHALL NOT be stored under the 13-character key `disp_interval`.
...
- **THEN** it SHALL return `enabled = false`, `rotation = 0`, and `interval = 60`

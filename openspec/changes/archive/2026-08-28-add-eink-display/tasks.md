...
- [x] 9.4a **Moved to change `assess-display-brownout-risk`.** The 100 µF bulk capacitor decision depends on the brownout observation, so both were split into a single follow-up rather than left open here
- [x] 9.5 Confirmed on hardware by the operator: with the display enabled and no sensor configured, the panel renders the `--.-°` / `-- %rH` placeholders rather than a stale or zero value
- [x] 9.6 Confirmed on hardware by the operator: `GET /api/status` reports internal free above the 20480 B OTA gate with the display enabled, and a full OTA update completed end to end. This is the check the whole paged-rendering design (D1) exists to satisfy — a full framebuffer would have failed it
- [x] 9.7 Confirmed on hardware by the operator: disabling via the UI blanks the panel during the save, before the restart, and no `disp_clear` key exists in NVS
- [x] 9.8 Confirmed on hardware by the operator: unplugging the panel produces three timed-out attempts, a single `ESP_LOGE`, then no further refresh attempts; no watchdog reset, and `GET /api/display` still reports `enabled: true` (the fault state is in-memory only and never rewrites the saved config)
- [x] 9.9 **Moved to change `assess-display-brownout-risk`.** The multi-hour `Reset reason:` watch for `BROWNOUT` is being investigated separately, together with the capacitor decision it informs
- [x] 9.10 Archive the change with `/opsx:archive` to create `openspec/specs/display/spec.md` and fold the four spec deltas into their existing capability specs
